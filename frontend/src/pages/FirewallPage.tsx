...
import { getChains, setDefaultAction, addRule, updateRule, deleteRule, reorderChain } from '../api/client';
import type { FirewallRuleset, FirewallRule } from '../types';
...
function SortableRuleItem({ rule, chainName, onDelete, onEdit, isWorking }: SortableRuleProps) {
  ...
  return (
    ...
    >
      ...
      <div className="w-32 font-mono text-xs text-gray-400 truncate">{rule.source_address || 'any'}{rule.source_port ? `:${rule.source_port}` : ''}</div>
      <div className="w-32 font-mono text-xs text-gray-400 truncate">{rule.destination_address || 'any'}{rule.destination_port ? `:${rule.destination_port}` : ''}</div>
...
export default function FirewallPage() {
  ...
  const buildRule = (fallbackNumber: number): FirewallRule => ({
    ...
    source_address: newRule.source_address || null,
    destination_address: newRule.destination_address || null,
          ...
          <div className="bg-gray-800 rounded-xl p-6 w-full max-w-lg border border-gray-700 shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input value={newRule.source_address || ''} onChange={e => setNewRule({...newRule, source_address: e.target.value || null})} className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded focus:outline-none focus:border-blue-500" placeholder="any / 10.0.0.0/24" />
                <p className="text-xs text-gray-500 mt-1">Leave empty for "any"</p>
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1">Dest Address</label>
                <input value={newRule.destination_address || ''} onChange={e => setNewRule({...newRule, destination_address: e.target.value || null})} className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded focus:outline-none focus:border-blue-500" placeholder="any" />
                <p className="text-xs text-gray-500 mt-1">Leave empty for "any"</p>
              </div>
              <div>
